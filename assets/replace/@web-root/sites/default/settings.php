 ...
 *
 * This file is managed by iqual/drupal-nuxt-platform and overwritten on every
 * scaffold run — do NOT customize it. Downstream files are loaded in this
 * order and are the place for project-specific settings:
 *
 * 1. settings.all.php (all environments, e.g. the hash salt; project-owned)
 * 2. settings.${APP_ENV}.php (per environment; created once, project-owned)
 * 3. services.${APP_ENV}.yml (per environment; project-owned)
 * 4. settings.platformsh.php (managed)
 * 5. settings.ddev.php (generated by DDEV)
 * 6. settings.extra.php (project-owned, loaded last, overrides everything)
 *
 * IMPORTANT NOTE:
...
//$config['system.site']['page']['front'] = '/node/29';
...
// The environment is exported as APP_ENV by the hosting environment
// (see .environment); default to local (e.g. DDEV, CI).
$env = getenv('APP_ENV');
$env = !empty($env) ? $env : 'local';

// Include settings that apply to all environments (e.g. the hash salt).
if (file_exists($app_root . '/' . $site_path . '/settings.all.php')) {
  include $app_root . '/' . $site_path . '/settings.all.php';
}

if (!empty($env)) {
  // Include settings that apply for a site and environment combination.
  if (file_exists($app_root . '/' . $site_path . "/settings.{$env}.php")) {
    include $app_root . '/' . $site_path . "/settings.{$env}.php";
  }
  // Include services that apply for a site and environment combination.
  if (file_exists($app_root . '/' . $site_path . "/services.{$env}.yml")) {
    $settings['container_yamls'][] = $app_root . '/' . $site_path . "/services.{$env}.yml";
  }
}
...
// Files that have no remaining usages will be deleted by cron
// @see https://www.drupal.org/node/2891902
$config['file.settings']['make_unused_managed_files_temporary'] = TRUE;

// Automatically generated include for settings managed by ddev.
$ddev_settings = $app_root . '/' . $site_path . '/settings.ddev.php';
...
/**
 * Include project-specific overrides last, so they can override any setting
 * above. This file is owned by the project and never scaffolded; settings.php
 * itself must not be customized.
 */
